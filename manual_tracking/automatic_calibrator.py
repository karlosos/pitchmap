...
class AutomaticCalibrator:
    ...
    def calibrate(self):
        if self.flag == False:
            ...
            regular_spacing = np.arange(1, self.manual_tracker.fl.get_frames_count(), 150)
            arg_min_x = self.camera_movement_analyser.arg_x_min
            arg_max_x = self.camera_movement_analyser.arg_x_max
            characteristic_frames = self.find_characteristic_frames(self.camera_angles)

            self.characteristic_frames_numbers = np.unique(np.sort(np.concatenate((regular_spacing, characteristic_frames, (arg_min_x, arg_max_x), (self.manual_tracker.fl.get_frames_count(),)))))
    ...
    def find_characteristic_frames(self, camera_angles):
        ...
        characteristic_frames = np.concatenate((min_peaks, max_peaks))
