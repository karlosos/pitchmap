...
class PlayerCircle(Circle):
    def __init__(self, player, radius, start_x, start_y, custom_position=None):
    ...
    def draw(self, win):
        pygame.draw.circle(win, self.color, [int(self.start_x + self.x), int(self.start_y + self.y)], self.radius, self.thickness)
