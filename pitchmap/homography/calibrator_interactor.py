...
class CalibrationInteractorCollectFrames(CalibrationInteractor):
    """
    This interactor is used only for collecting frame, auto homography, guessed homography tuples for collecting for automatic
    outlier detection
    """
    def __init__(self, pitch_map, calibrator, frame_loader):
        super().__init__(pitch_map, calibrator, frame_loader)
    ...
    def find_characteristic_frames(self, camera_angles):
        selected_frames = np.arange(0, len(camera_angles), 10)
        return np.sort(selected_frames)
    ...
    def start_calibration(self):
        ...
        if self.__current_calibration_frame_idx >= 0:
            ...
            # TODO: check if found transformation is good/bad. Automatically accept or reject potential homography.
            return True
        return False
    ...
    def accept_transform(self):
        self.__homographies_for_characteristic_frames.append(self.__current_homography)
        self.__homography_flags.append(True)
        if not self.start_calibration():
            self.interpolate()

    def cancel_transform(self):
        self.__homographies_for_characteristic_frames.append(self.__current_homography)  # self.__current_homography should be None too,
        self.__homography_flags.append(False)
        # but for a "safety" reason I wanted to be explicit
        if not self.start_calibration():
            self.interpolate()
    ...
    def interpolate(self):
        ...
        # TODO: save detected homographies, flags, and homographies guessed from angle
        flags = self.__homography_flags
        false_indices = [i for i, x in enumerate(flags) if x == False]
        characteristic_frame_numbers = self.__characteristic_frames_numbers
        guessed_homographies = [self.get_homography(frame_number) for frame_number in characteristic_frame_numbers]
        from pitchmap.cache_loader.pickler import pickle_data
        pickle_data((characteristic_homographies, guessed_homographies, flags, false_indices), "./data/cache/characteristic_frames.pickle")
